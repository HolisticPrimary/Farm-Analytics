...
**ขั้น 3 — เทียบกับ Ross 308** (SECONDARY, อ้างอิงเท่านั้น):
```
rossFeed = ROSS308[age].daily
rossDev (%) = (feedPerBird − rossFeed) ÷ rossFeed × 100
```
...
stdBw  = MIXED_BW[age]               ← น้ำหนักมาตรฐาน "คละเพศ" รายวัน
                                       (PRIMARY, จากชีท "คละเพศ" ของฟาร์ม)
devPct = (wt_age − stdBw) ÷ stdBw × 100
```

ตัวเทียบรอง (แสดงเทียบให้เห็นในตาราง):
```
rossBw  = ROSS308[age].bw
rossDev = (wt_age − rossBw) ÷ rossBw × 100
```

| devPct (เทียบเกณฑ์คละเพศ) | สถานะ |
|---|---|
| < −5% | **BEHIND** (โตช้า) |
| −5% ถึง +5% | **ON** (ตามเกณฑ์) |
| > +5% | **AHEAD** (โตเร็ว) |

> เกณฑ์คละเพศที่ฟาร์มกำหนดเองอยู่ที่ ~+1 ถึง +4% สูงกว่าค่า Ross 308 ในช่วงต้น-กลางรอบ
> ทำให้การประเมินเข้ากับสภาพไก่ในฟาร์มไทยมากกว่า — ใช้ตัวเลขนี้เป็นหลักในการตัดสิน
